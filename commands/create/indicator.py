...
from commands.create.constants import INDICATOR, PAGINATION_LIMIT, indicators
...
async def input_indicator(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        ...
        await update.effective_message.reply_markdown_v2(text="Select indicator", reply_markup=reply_markup)
    return INDICATOR
...
async def new_indicator(update: Update, context=ContextTypes.DEFAULT_TYPE):
    keyboards = [
        [InlineKeyboardButton("Done", callback_data="hihihihi")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboards)
